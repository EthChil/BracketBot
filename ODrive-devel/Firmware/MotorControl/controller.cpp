...
{
    auto& cogmap = config_.anticogging.cogging_map_f;
 ...
 */
bool Controller::anticogging_calibration_f(float pos_estimate, float vel_estimate) {
    ...
    if (std::abs(pos_err) <= config_.anticogging.calib_pos_threshold / (float)axis_->encoder_.config_.cpr &&
        ...
        config_.anticogging.cogging_map_f[std::clamp<uint32_t>(config_.anticogging.index++, 0, 3600)] = vel_integrator_torque_;
    }
...
bool Controller::update() {
    ...
    if (config_.anticogging.calib_anticogging_f) {
        if (!anticogging_pos_estimate.has_value() || !anticogging_vel_estimate.has_value()) {
            set_error(ERROR_INVALID_ESTIMATE);
            return false;
        }
        // non-blocking
        anticogging_calibration_f(*anticogging_pos_estimate, *anticogging_vel_estimate);
    }
    ...
    if (config_.anticogging.calib_anticogging_b) {
        ...
        // non-blocking
        anticogging_calibration_b(*anticogging_pos_estimate, *anticogging_vel_estimate);
    }
