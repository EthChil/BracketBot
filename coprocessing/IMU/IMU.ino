...
void setReports() {
  ...
  if (!bno08x.enableReport(SH2_ROTATION_VECTOR, 250)) {
    Serial.println("Could not enable stabilized remote vector");
  }
 if (!bno08x.enableReport(SH2_GYROSCOPE_CALIBRATED, 250)) {
  ...
  if (bno08x.getSensorEvent(&sensorValue)) {
      switch (sensorValue.sensorId) {
        case SH2_ROTATION_VECTOR:
          quaternionToEulerRV(&sensorValue.un.rotationVector, &ypr, false);
          
          break;

        
        case SH2_GYROSCOPE_CALIBRATED: // Best angle reading
    ...
    // Serial.print(String(ypr.yaw, 4) + " ");
    // Serial.print(String(ypr.pitch, 4) + " ");
    // Serial.print(String(ypr.roll, 4) + " ");

    // Serial.print(String(dypr.yaw, 4) + " ");
    // Serial.print(String(dypr.pitch, 4) + " ");
    // Serial.println(dypr.roll, 4);
    static long last = 0; 
    long now = micros();
    // Serial.print(String(now-last) + " ");
    last = now;
    Serial.println(String(ypr.pitch, 6)+' '+String(ypr.yaw, 6)+' '+String(dypr.pitch, 6)+' '+String(dypr.yaw, 6));
    
    // float pitch_rate = -(ypr.pitch - pitchPrev)/deltaT; // Negative to match gyro orientation
    // float yaw_rate = (ypr.yaw - yawPrev)/deltaT;
    String msg = 's'+String(ypr.pitch, 6)+','+String(ypr.yaw, 6)+','+String(dypr.pitch, 6)+','+String(dypr.yaw, 6) + 'e';
    Serial1.print(msg);
